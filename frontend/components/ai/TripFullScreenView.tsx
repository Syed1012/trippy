...
export default function TripFullScreenView({
  ...
  const [expandedDays, setExpandedDays] = useState<Set<number>>(() => {
    if (trip.aiItinerary?.length) {
      return new Set(trip.aiItinerary.map(d => d.dayNumber));
    }
    return new Set([1]);
  });
  ...
  async function generateItinerary() {
    ...
    try {
      ...
      if (data.dailyPlan?.length) {
        ...
        setExpandedDays(new Set(data.dailyPlan.map((d: any) => d.dayNumber)));
      }
    } catch (err) {
  ...
  async function handleChatSend(inputOverride?: string) {
    ...
    try {
      ...
      if (data.hasModification && data.updatedItinerary) {
        ...
        setExpandedDays(new Set(data.updatedItinerary.map((d: any) => d.dayNumber)));
      }
    } catch {
                                        ...
                                        {isEditingThis ? (
                                          ...
                                          <div className="rounded-xl border border-border/50 bg-white hover:border-trippy-300/60 hover:shadow-sm transition-all px-4 py-3 mb-2 group/card relative">
                                            ...
                                            <div className="flex items-start gap-3 pr-14">
                                              {/* Category Icon Badge */}
                                              {(() => {
                                                const IconComponent = CAT_ICONS[cat.toUpperCase()] || MapPin;
                                                const colorCls = CAT_COLORS[cat.toUpperCase()] || "bg-gray-100 text-gray-600 border-gray-200";
                                                return (
